...
5b. **Invoke Monitor at session entry (Claude Code only — builder-side).** When running under Claude Code v2.1.98+, invoke the built-in Monitor tool once at session entry with the canonical builder-side pattern documented in `BUILDER.md` §Start Monitor at session entry (MON-003). Monitor tails `multicheck/agentchat.md` for reviewer verdicts + HITL entries and wakes you automatically on matches — replacing the manual `check chat` relay for the Claude side. If Monitor invocation fails or terminates mid-session, post `STATE: monitor-dead` and fall back to manual-relay pattern for the rest of the session. Codex / non-Claude builder sessions keep manual relay (no Monitor equivalent yet).
...
11. **Pre-flight before every story** — mandatory. Before writing any code on a new story, post a `[S-NNN]` pre-flight entry answering 7 questions: (1) goal fit, (2) branch topology with `git merge-base` verification, (3) file targets with existence + rename check, (4) scope declaration in `details.md`, (5) value-set parity across all layers (DB / Drizzle / TS / Zod / switches / OpenAPI), (6) full end-gate command + baseline + risk prediction, (7) reconnaissance (trace imports, survey sibling tests, identify factory patterns, map ESM/jest transform boundaries, survey existing mocks, enumerate propagation layers, list invariant categories). Wait for `[R-NNN] DECISION: accept` on the pre-flight before proceeding to code. The reviewer will reject any later work that refers to code written before the pre-flight ack. See `BUILDER.md` "Pre-flight questions" for the full spec with commands to run. Cost ~2-5 minutes per story; prevents hours of rework from stale-branch-base, wrong-file-target, silent-scope-expansion, cross-layer-drift, and harness-stubbing-cascade failure modes.

12. **Harness-failure triage before reflexive stubbing** — mandatory. When a test fails for harness reasons (import failure, missing mock, type mismatch at test boundary), you MUST work through the 5-step triage framework BEFORE adding any `jest.mock()` / `sinon.stub()` / test-boundary override: (1) existing factory/helper in workspace? (2) sibling test with matching mock pattern? (3) product-code shape problem that should be refactored? (4) test at the wrong boundary? (5) only if 1-4 say no, stub with a comment explaining which options you ruled out. Output the triage as a `HARNESS TRIAGE:` block in your next `[S-NNN]` entry so the reviewer can verify. Reflexive stubbing without triage is a process violation. See `BUILDER.md` "Harness-failure triage framework" for the full decision tree.

13. **Dress rehearsal before first commit** — run your pre-commit hook manually (`sh .husky/pre-commit` or equivalent) against your staged files before typing `git commit`. Catches Prettier and contract-scan failures cheaply. Treating `git commit` as your first test run creates repeated stops.
