<!--
Example: project-level PR promotion gate — claims-monorepo

Source: claims-monorepo/specs/pr.md as of 2026-04-09 (563 lines)
License: pulled from the claims-monorepo project with permission of the
project owner; shown here as a reference implementation

======================================================================
WHAT THIS EXAMPLE SHOWS
======================================================================

A comprehensive project-level PR promotion gate that mirrors the multicheck
review discipline at the project level. It evolved over 2 days of live
sessions (198 lines → 362 lines → 563 lines) and reflects the learnings
from [R-028] (diff-content check), [R-035] (topology), [R-038] (scope
declaration), and [R-040] (cross-layer value consistency) inline.

This is Layer 2 in the multicheck 3-layer architecture (upstream reference
→ project anchor → session state). pr.md is a project-owned promotion gate
that lives alongside multicheck's upstream rules. It does NOT replace
multicheck — it's the project-specific elaboration of what promotion means
for claims-monorepo.

======================================================================
WHICH SECTIONS ARE UNIVERSAL
======================================================================

Sections you could adapt for your own project's PR gate:

  §0.5 Branch topology — the mandatory merge-base check before any
       promotion work. Universal and now upstream via pre-flight Q2 and
       hooks/pre-push.sh.

  §1 Slice purity — `git diff --name-only A..B` against declared in-scope
       files. Universal. Reviewer runs this as part of verification order.

  §11e Cross-layer value consistency — enum/type/schema parity across DB /
       Drizzle / TS union / Zod / switches / OpenAPI / fixtures. Universal
       failure mode; reviewer-time version queued as multicheck PENDING #3,
       write-time version (propagation manifest) targeted for v0.5.1+.

  §11f Specialist-persona sweep — reviewer wears 5 hats (security /
       performance / architecture / style / cross-layer) for the final
       review pass. Universal structural discipline; queued as multicheck
       PENDING #4.

  §11g External asymmetric reviewer — trigger an independently-prompted
       external reviewer (e.g. code-review-swarm) on the final head before
       `gh pr ready`. Universal protection; documented in multicheck
       README.md 3-layer architecture + meta-observation M4.

======================================================================
WHICH SECTIONS ARE PROJECT-SPECIFIC
======================================================================

Do NOT copy these verbatim — they're claims-monorepo specific:

  §11a Hook gate count — the 59 suites / 700 passed / 9 skipped / 3 todo
       numbers are specific to claims-monorepo's contract test suite.
       Your project will have different counts.

  §11b Prettier extension allow-list — claims-monorepo-specific file
       extension whitelist.

  §11c Squash preview specifics — references to claims-monorepo's specific
       squash workflow and tools.

  §11d Correction-round budget — the per-PR correction round limit is
       claims-monorepo-specific scheduling discipline.

  §2 "Files currently in scope for #NNN" — references to claims-monorepo
       tickets and the project's specific ticket numbering.

  References to specific commits (f15c4ae8, 325095ea, etc.) and specific
       reviewer entries ([R-028], [R-035], etc.) are session-specific
...
1. Read the whole file to understand how a project-level PR gate can
   layer on top of multicheck's upstream rules.

2. For your own project, create a `specs/pr.md` (or equivalent) that:
   - Starts from the universal sections above (topology, slice purity,
     cross-layer, persona sweep, external reviewer)
   - Adds your project-specific sections (hook gate details, file
     extension rules, ticket schema)
   - Evolves over time as you encounter project-specific failure modes
     and add sections referencing your own reviewer entries

3. The pr.md file is Layer 2 — it persists across sessions but is
   owned by the project, not multicheck upstream. When multicheck ships
   a new universal rule (e.g. v0.5.0 pre-flight Q7 reconnaissance), you
   can either adopt it by adding a reference from pr.md to the upstream
   rule, or wait for the next session's Phase 0 refresh to pick it up
   automatically via CLAUDE.md / AGENTS.md.

4. The case study at case-studies/2026-04-08-claims-monorepo-calendar-
   consolidation/ includes the evolution of this file from 198 lines
   (pre-§11a) through 362 lines (post-§11d) to the current 563 lines,
   so you can see what drove each addition.
